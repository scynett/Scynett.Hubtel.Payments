...
| 4 | Callback endpoint returns 401 / 503 / 500 where it used to return 400 | Review dashboards, alerts, and any WAF/proxy that reacts to 5xx on the callback route. |
...
| **Callback status codes 400 → 401/503/500** | **Behaviour changes, beneficially.** Nothing in the consumer depends on 400; no WAF/proxy/alert rule keys off it. `CallbackValidationOptions` is never configured, so `EnableValidation` is off and the **401 path is unreachable**. The **503** (in-flight) and **500** (transient) changes mean Hubtel will now retry callbacks it previously dropped — the payment gets reconciled instead of hanging. Note `Program.cs` request logging maps `>= 500` to `Error`, so expect some new Error-level log lines that are actually healthy retries. |
